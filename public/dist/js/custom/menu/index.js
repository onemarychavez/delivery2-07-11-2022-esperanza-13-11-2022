...
const modal = $('#modal')
const alerta = $('#modal2')
const alertaRemove = document.getElementById('btnDelete')
...
const removeMenu = async (menuid)=>{
    try {
        const request = await fetch(`/menu/${menuid}`,{
            method:"DELETE"
        })
...
document.addEventListener("DOMContentLoaded", (event)=> {
    ...
    alertaRemove.onclick = (e)=>{
        if(menueliminar >0){
            removeMenu(menueliminar)
        }
    }
